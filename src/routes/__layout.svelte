...
import { feature } from "@turf/helpers";
  ...
  // We geolocate the user's IP address, to initialize the map settings near where they are and center the map on a familiar place.
   ...
   async function ipToCoordinates() {
    ...
    async function fetchAlternativesGeometery() {

      console.log($map_center_store);

    // var point = turf.point($map_center_store);
    var point =     {
      "type": "Feature",
      "properties": {},
      "geometry": {
    ...
    // console.log(point);
    var bikeshareBuffer = turfBuffer(point, 20, {units:'miles'});
    var transitBuffer = turfBuffer(point, 2, {units:'miles'});
...
// console.log(bikeshare_transit_geojson.filter(feature => feature.properties?.amenit != "bicycle_rental" && !feature.properties?.shop == "bicycle"));
// const fetch_bikeshare = await fetch(`https://serene-journey-42564.herokuapp.com/https://overpass.kumi.systems/api/interpreter?data=${overpass_bikeshare_query}`, {

// // await fetch(`https://overpass.kumi.systems/api/interpreter?data=[out:xml][timeout:25000];(${osm_type}(${location_data.features[0].properties.osm_id}););out geom;`, {
...
// // .then(data => { console.log(data); geometry_xml = data } );

// const bikeshare = await fetch_bikeshare.json();

// // console.log(json);

// let bikeshare_geojson = osmtogeojson(bikeshare);

// console.log(bikeshare_geojson);

// $bikeshare_store = bikeshare_geojson;


// const fetch_transit = await fetch(`https://serene-journey-42564.herokuapp.com/https://overpass.kumi.systems/api/interpreter?data=${overpass_transit_query}`, {
...
// const transit = await fetch_transit.json();

// // console.log(json);

// let transit_geojson = osmtogeojson(transit);

// console.log(transit_geojson);

// $transit_store = transit_geojson;


// console.log(geometry_xml);

// let geometry_geojson = osmtogeojson(geometry_xml);

// console.log(geometry_geojson);

// geometries = false;

// for (var i=0; i < geometry_geojson.features.length; i++) {
// geometry_geojson.features[i].properties.osm_type_id = `${geometry_geojson.features[i].properties.id.charAt(0).toUpperCase()}${geometry_geojson.features[i].properties.id.substr(9)}`
// }
...
function navigate (next) {
  ...
  async function showNearbyBikesAndTransit() {
    ...
    var bikeshareBuffer = turfBuffer(point, 0.25, {units:'miles'});
    var transitBuffer = turfBuffer(point, 0.25, {units:'miles'});
